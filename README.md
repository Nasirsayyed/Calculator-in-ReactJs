...
**Settings** — theme (Light / Dark / AMOLED / High Contrast), 8 accent colors, font size, button size, animation speed, decimal precision, angle mode, history limit, layout density, sound effects (synthesized, no audio assets), haptic feedback (Vibration API), reduce‑motion override, a default-calculator picker (or "remember last used"), a language picker, reset to defaults.

**Internationalization** — English, Spanish, French, and Arabic (RTL), switchable from Settings. Covers the app chrome: Navbar, Footer, Sidebars, Settings panel, and the "Browse calculators" launcher (search, categories, coming-soon state). Arabic flips `<html dir>` to `rtl`, and the existing flexbox/grid layout mirrors correctly with no additional CSS overrides needed. Individual calculator pages' field labels are still English-only - see [Roadmap](#roadmap).
...
**Route-based code splitting**: all 32 utility calculator pages (everything except the Standard/Scientific calculator, which is the landing route and stays eagerly bundled) are loaded via `React.lazy()` in `App.tsx`, each wrapped in its own `<Suspense fallback={<RouteLoadingFallback />}>`. Every page compiles to its own small chunk (roughly 0.3–3.7KB) fetched only on first visit to that route, which cut the main JS bundle from ~935KB to ~707KB. Verified in a real browser: the BMI page's chunk is not requested until `/bmi` is visited (via both a full navigation and client-side SPA routing), and it loads correctly with no console errors either way.
...
i18n covers the app chrome and every individual calculator page's field labels, section titles, and result rows (see [Features](#features)) across all 4 languages, including RTL Arabic. Pure mathematical/technical notation (single-letter algebra variables like `a`, `b`, `x`; matrix/vector operator symbols like `A × B`, `Aᵀ`, `|A|`; bitwise operator names `AND`/`OR`/`XOR`/`NOT`) is intentionally left untranslated, since it reads the same across languages.
